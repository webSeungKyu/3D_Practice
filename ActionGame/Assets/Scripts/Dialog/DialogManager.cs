...
public sealed class DialogManager
{
    ...
    DialogController _currentDialog; // 현재 사용 중인 다이얼로그
    ...
    {
        //다이얼로그를 리스트에서 첫 번째 값을 가져오겠습니다
        DialogData next = _dialogQueue[0];

        //가져온 값의 형태를 확인해 어떤 컨트롤러인지를 확인합니다
        DialogController contorller = _dialogMap[next.Type].GetComponent<DialogController>();

        //조회한 다이얼로그 컨트롤러를 현재의 다이얼로그 컨트롤러로 지정합니다
        _currentDialog = contorller;
